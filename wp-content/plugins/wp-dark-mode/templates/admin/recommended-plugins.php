...
$plugins = [
    ...
    [
        ...
        'name' => 'Webinar and Video Conference with Jitsi Meet',
    ],    
    [
        'slug' => 'stock-notifier-for-woocommerce', 
...
$plugins_data = array_map(function($plugin) {
    ...
    if(isset($plugin['description'])) {
        $data->short_description = $plugin['description'];
    }
...
foreach ($plugins_data as $plugin) {
    $recommended_plugins[$plugin->slug] = $plugin;
}
            ...
            $('a.hide-recommended-btn').on('click',function(event){
                ...
                $( "#hide-recommeded-plugins" ).dialog({
                    ...
                    buttons: {
                        "Hide it": {
                            click: function () {
                                window.location = "<?php echo admin_url('admin.php?page=wp-dark-mode-settings&hide_wp_dark_mode_recommended_plugin=1&nonce='.wp_create_nonce("wp_dark_mode_recommended_plugin"));?>";
                            },
                            text: 'Hide it',
                            class: 'btn red-btn'
                        },
